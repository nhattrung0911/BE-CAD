...
Status as of the last Codex session:

- Backend tests: `cd backend; python -m pytest -q -p no:cacheprovider` passed with 60 tests.
- Migration gate: `cd backend; python -m alembic upgrade head` passed on the local SQLite dev database.
- Compose config gates: `cd infra; docker compose --env-file .env.example config --quiet` and `docker compose --env-file .env.production.example config --quiet` passed.
- Frontend demo build: `cd frontend-demo; npm.cmd run build` passed, with only the expected Three.js bundle-size warning.
- Local demo was verified with `CAD_BACKEND=cadquery`, `MODEL_SYNC_GENERATION=true`, and a demo-only `TEMPLATE_VERSION=cadquery-demo-v1`; all five sample products returned binary GLB payloads with `glTF` headers.

Important implementation state:

- `frontend-demo/` is still a thin BE contract/demo harness, not production frontend.
- Vendor exact assets are served only when `license_status=approved` and `validation_status=valid`; `pending` assets are ingested but not selected by the resolver.
- S3 deployments should set `S3_PUBLIC_BASE_URL` to the CDN/public asset base URL. Without it, storage falls back to the configured public prefix.
- `batch_pregenerate` dispatch now targets the batch Celery task and wraps a single job id as a one-item batch.
- Production-style Docker runtime is not yet fully proven on this workstation; previous `docker compose up -d --build` attempts timed out before container creation. Treat Docker image/runtime smoke as the next unresolved backend infrastructure gate.
...
This project is indexed by GitNexus as **CAD** (649 symbols, 1495 relationships, 46 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
...
3. `READ gitnexus://repo/CAD/process/{processName}` — trace the full execution flow step by step
...
| Resource | Use for |
|----------|---------|
| `gitnexus://repo/CAD/context` | Codebase overview, check index freshness |
| `gitnexus://repo/CAD/clusters` | All functional areas |
| `gitnexus://repo/CAD/processes` | All execution flows |
| `gitnexus://repo/CAD/process/{name}` | Step-by-step execution trace |
